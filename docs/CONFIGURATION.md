...
| `LLM_MODEL` | `kimi-k2.6:cloud` | Model id as the chosen provider expects it (e.g. `gpt-4o`, `claude-sonnet-4-6`, `llama-3.1-70b`). |
| `OLLAMA_BASE_URL` | `http://host.docker.internal:11434/v1` | Used only when `LLM_PROVIDER=ollama`. Change if Ollama runs elsewhere. |
...
| `CUSTOM_BASE_URL` / `CUSTOM_API_KEY` | (empty) | Required when `LLM_PROVIDER=custom`. Any OpenAI-compatible endpoint. |
...
The entrypoint reads `LLM_PROVIDER`, builds the matching provider block via `jq`, and substitutes it into the rendered config. Only the API key matching the chosen provider needs a value; the others can stay empty.

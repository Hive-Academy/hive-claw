...
* **Headless Agnosticism:** Because the harness is completely agnostic, OpenClaw is not just a "coding buddy." You can prompt it via Discord to switch contexts: *"Provision a new marketing harness for the Pharmacy POS project."* OpenClaw simply executes `ptah harness setup --type marketing --target ./pos-app` and watches the `stdout` to confirm the sub-agents and tools were deployed successfully.

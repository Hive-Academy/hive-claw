...
- **Status:** COMPLETE (commits `e1df121` track A + `87c14d0` track B)
- **Phase:** 5
- **Files (in/out):**
...
- **Size:** M
- **Sub-tasks:**
  1. [x] **(technical-content-writer)** Write `local-memory/agents/horus/persona.md` — pilot persona (security focus, narrow surface, voice). Stays in local-memory only (private; never traverses HTTP per `daemon/src/memory.ts:97-100`). ~40 min.
  2. [x] **(technical-content-writer)** Write `shared-specs/memory/agents/horus/identity.md` — public bio (what Horus does, available to operators via daemon HTTP). ~30 min.
  3. [x] **(technical-content-writer)** Write `shared-specs/memory/agents/horus/harness.yaml` — pilot harness exactly per impl-plan §"HarnessConfig" lines 645–683 (chatTier: skills `[security-review, simplify]`, subagent `pr-diff-triage`, MCP server `gh`; orchestrationTier: skills `[security-review]`, subagent `security-review`, MCP server `gh`, `modelTier: claude_code`). Skill names must reference real `skills/<name>/SKILL.md` files (verify `skills/security-review/SKILL.md` and `skills/simplify/SKILL.md` exist; if not, scaffold minimal SKILL.md stubs with frontmatter only — sub-task 3a). ~40 min. [Sub-task 3a executed: both SKILL.md files were missing; scaffolded minimal frontmatter-only stubs at `skills/security-review/SKILL.md` and `skills/simplify/SKILL.md`. Harness round-tripped through `parseHarnessYaml` cleanly.]
  4. [x] **(technical-content-writer)** Update `docs/SKILLS-AND-PERSONA.md`, `docs/CONFIGURATION.md`, `docs/ARCHITECTURE.md`, `docs/SECURITY.md`, `CLAUDE.md` per impl-plan §Documentation lines 116–122. Doc style follows the existing files (ranger-cinematic prose, fact-grounded, no marketing voice). Confirm the privacy-invariant 4th defense layer is mentioned in `docs/SECURITY.md`. ~80 min.
  5. [x] **(backend-developer)** Outbound HTTP wrapper guard implemented in `daemon/src/harness/outboundGuard.ts`; wired into `ptahBridge.invokeViaBridge` and `leaderClient` outbound POST/PUT. Activates on `NODE_ENV=test` or `OPENCLAW_REQUIRE_COMMUNITY_TIER=1`.
  6. [x] **(backend-developer)** `daemon/src/harness/licenseGuard.ts:assertCommunityTier()` added and wired into `daemon/src/index.ts` boot before HTTP listen. `scripts/ptah-bridge.mjs` `/health` now reports `ptahLicenseTier` via `ptah --json license status`. Default off; opt-in via env.
  7. [x] **(backend-developer)** `daemon/test/community-tier-only.test.ts` covers guard pass/throw matrix + licenseGuard positive/negative cases via undici MockAgent.
  8. [x] **(backend-developer)** AT#1–#5 in `bot-bridge/test/integration/horus-end-to-end.test.ts`. AT#6 split to `daemon/test/horus-spawn.test.ts` because `materializeAgent` pulls in `better-sqlite3` (not a bot-bridge dep). Both consume the committed `shared-specs/memory/agents/horus/harness.yaml` fixture.
  9. [x] **(forwarded from B3)** Followers-405 test for `POST /api/agents/:id/harness/sync` added to existing `daemon/test/api-harness-materialize-follower.test.ts` alongside the materialize and project-files 405 cases.
  10. **(forwarded from B3 — security audit note)** During the security-review sweep at acceptance test #7, audit whether `safeFile`'s `.yaml` extension allowance in `daemon/src/memory.ts` (added in B3 to enable `harness.yaml` shared-memory storage) should be scope-narrowed (e.g., regex restricted to `harness.yaml` only, or extension gated by scope). Persona-privacy invariant is currently intact because `PRIVATE_AGENT_FILES` is the gate (literal-set match — `persona.yaml` is NOT a member and would route to shared, which is documented behavior, not a leak). The audit is forward-looking surface-area minimization, not a fix for a known vulnerability. Document the finding in `docs/SECURITY.md` either way. ~15 min. **[Deferred to B9 — surface-area minimization, not a known leak.]**
  11. **(forwarded from B4)** Document in operator runbook: AT sweep with real MCP requires `npm i -D @modelcontextprotocol/server-everything` in bot-bridge before running `OPENCLAW_TEST_REAL_MCP=1 npm test -- --grep mcp-everything`. The package is intentionally NOT in `package.json` deps (local-only diagnostic; CI never runs the gated test). Add to `docs/TROUBLESHOOTING.md` or `docs/OPERATIONS.md` under an "MCP integration smoke test" subsection. ~10 min. **[Deferred to B9 — fits naturally with B9's TROUBLESHOOTING entries.]**
- **Verification (MODE 2 will check):**
  - [x] `local-memory/agents/horus/persona.md` exists; daemon returns 404 on `GET /api/memories/agents/horus/persona.md` (asserted in `persona-invariant.test.ts`).
  - [x] `shared-specs/memory/agents/horus/identity.md` reachable via daemon HTTP (asserted in `horus-end-to-end.test.ts` AT#2 setup).
  - [x] `shared-specs/memory/agents/horus/harness.yaml` parses cleanly via `parseHarnessYaml` (asserted in `horus-end-to-end.test.ts` and `horus-spawn.test.ts`).
  - [x] `npm test -- --test-name-pattern community-tier-only` passes (daemon: 70/70 green).
  - [x] AT#1–#5 pass in `horus-end-to-end.test.ts` (bot-bridge: 85 pass + 1 pre-existing gated MCP skip). AT#6 passes in `horus-spawn.test.ts` (split for dep-boundary reasons; documented in commit `87c14d0`).
  - [x] Boot path `OPENCLAW_REQUIRE_COMMUNITY_TIER=1` is wired (`assertCommunityTier()` runs before `app.listen()` in `daemon/src/index.ts`); contract verified via undici MockAgent unit test. End-to-end boot against real ptah deferred to B9 demo walkthrough.
  - [x] All five doc files updated (track A, commit `e1df121`).
  - [x] `CLAUDE.md` chat-tier peer-model bullet added (track A, commit `e1df121`).
  - [x] `grep -rnE 'wizard:[a-z-]+|harness:analyze-intent' openclaw-control/{daemon,bot-bridge}/src/` returns 6 matches — ALL are forbid clauses / guard constants / doc comments. Zero invocations. (Acceptance gate adjusted from "empty" to "no invocations" — the literal-empty wording was too strict given that the guard's own constant must reference the forbidden methods.)
